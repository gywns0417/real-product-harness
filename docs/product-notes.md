...
- Runtime UX is conversational first: plain text talks to the connected AI agent, while slash commands control workflow state. Recognized start-workflow requests render a deterministic execution-plan card and persist the shown control as a runtime intent. Exact execution phrases such as `confirm` or `이 계획 실행해줘` confirm only the last presented intent; external writes and user-approval commands stay behind their explicit gates.
