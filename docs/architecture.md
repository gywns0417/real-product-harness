...
1. `/setup auto` is the Hermes-style boot wizard: it asks for the desired AI agent and MCP servers, accepts missing env values, writes secrets only to `.env`, refreshes `.rph/config.json` and `.mcp/config.json`, and immediately probes the selected connections.
